    ...
    while(input_file.is_open()){
        if(input_file.eof()){
            ...
            while(getline(input_file, line)){
                ...
                // first test : a-b,c-d   c---a--b----d    a-b is contained in c-d
                if ( ((section_IDs[0] >= section_IDs[2]) && (section_IDs[0] <= section_IDs[3])) &&  
                     ((section_IDs[1] >= section_IDs[2]) && (section_IDs[1] <= section_IDs[3])) ) { 
                        ++pair_nb;
                        std::cout << line <<" | line nb : " << line_nb << std::endl;
                    ++line_nb;
                    section_IDs.clear();
                }
                // second test : a-b,c-d   a---c--d----b    c-d is contained in a-b
                else if ( ((section_IDs[2] >= section_IDs[0]) && (section_IDs[2] <= section_IDs[1])) &&  
                     ((section_IDs[3] >= section_IDs[0]) && (section_IDs[3] <= section_IDs[1])) ) { 
                        ++pair_nb;
                        std::cout << line <<" | line nb : " << line_nb << std::endl;
                    ++line_nb;
                    section_IDs.clear();
