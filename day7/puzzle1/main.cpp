...
std::vector<Directory *> search_for_directorie_sizes(   std::vector<Directory *>  & list_dir_ok_size, 
                                                        ...
                                                        int max_size_required){
    if(root_directory->total_size <= max_size_required){
    ...
    for(auto dir : root_directory->directory_list){
        search_for_directorie_sizes(list_dir_ok_size, dir, max_size_required);
    }
...
int main(int argc, char * argv[]){
    ...
    while(input_file.is_open()){
        if(input_file.eof()){
            ...
            list_dir_ok_size = search_for_directorie_sizes(list_dir_ok_size, &root_dir, 100000);
            int s = 0;
            for(auto elem : list_dir_ok_size){
                s += elem->total_size; 
                std::cout << "Dir name : " << elem->directory_name << " | Size : " << elem->total_size << std::endl;
            }
            std::cout << "Total size of directories with max_size " << 100000 << " : " << s << std::endl;
        }
    }
    return 0;
}
